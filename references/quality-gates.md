...
**修复**：intro 只通过平台 CLI 推送管理，SKILL.md 不保留
...
### AP-21：frontmatter/metadata 含真实人员信息
**症状**：frontmatter 中保留 `metadata.platform.creator: "username"` 等平台专属字段
**后果**：公开 Skill 发布后任何人均可看到发布者真实身份信息，造成个人信息暴露
**检查**：`grep -n 'metadata\.' SKILL.md`（检查是否有平台专属字段）
**修复**：删除整个 metadata 块；发布者信息由广场系统自动记录，无需手动维护

### AP-22：_meta.json 含真实凭据未排除
**症状**：Skill 目录下存在 `_meta.json`，其中含真实 appkey 或作者信息，且没有 .skillignore 排除
