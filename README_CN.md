...
每条标注根因事故和对应设计原则，不是"禁止X"的死记令，而是"为什么X会翻车"的经验提炼。按 5 类分组：

**结构膨胀类（5条）**

| # | 反模式 | 根因事故 |
|---|---|---|
| 1 | >250行膨胀 | 猎手418行Agent忽略后半段指令 |
| 4 | 多跳引用A→B→C | 面评虾3层嵌套Agent遗忘 |
| 9 | IRON LAW是套话 | 多个Skill写"shebang+行数"被忽略 |
| 13 | 无数据量限制 | 小雷达91条串行查询超时 |
| 15 | 大文件无截断 | read大文件context溢出 |
...
| 5 | 无完成门控 | audit跑完不确认就交付 |
| 7 | 无Confirmation Gate | 6/22未等确认自作主张发布 |
| 8 | 无Pre-Delivery Checklist | 发布后才发现漏安全扫描 |
| 12 | 循环无终止条件 | 6/17串行CLI无上限超时 |

**规则质量类（4条）**

| # | 反模式 | 根因事故 |
|---|---|---|
| 2 | description写宣传语 | description写"高效便捷"无法触发 |
| 6 | 不可验证指令 | "高质量面评"Agent自认合格 |
| 10 | 无AP列表 | 新Skill反复犯同类错误 |
| 19 | 规则只有MUST没WHY | Agent死记禁止令遇边界选错 |

**一致性类（4条）**

| # | 反模式 | 根因事故 |
|---|---|---|
| 14 | 冗余重复提示 | SKILL.md+references两处写同一规则 |
| 16 | SKILL.md留intro | 修改SKILL.md的intro线上没同步 |
| 18 | 重复造轮子 | 多Skill各自SSO换票改一处漏一处 |
| 20 | IRON LAW照搬通用模板 | IRON LAW5条套话Agent全忽略 |

**工程规范类（2条）**

| # | 反模式 | 根因事故 |
|---|---|---|
| 11 | 无子Agent执行规范 | 子Agent跳步/自创格式 |
| 17 | Shell无shebang/set-euo | 脚本静默失败Agent以为成功 |
