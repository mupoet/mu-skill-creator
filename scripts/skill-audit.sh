#!/bin/bash
# skill-audit.sh — Skill structure health checker
#
# 用法:
...
#   SENSITIVE - 是否引用了敏感系统URL
#   LINES     - 行数（>250 标黄，信息项不阻断）

set -euo pipefail

SKILL_BASE="${SKILL_BASE:-${HOME}/.catpaw/skills}"
VERBOSE=false
TARGETS=()
...
printf "%-25s %5s %8s %8s %10s %9s\n" "SKILL" "LINES" "IRON_LAW" "IL専属" "DESC" "SENSITIVE"
printf "%-25s %5s %8s %8s %10s %9s\n" "-------------------------" "-----" "--------" "--------" "----------" "---------"
...
for entry in "${SKILLS[@]}"; do
  ...
  # Security: sensitive system URL patterns
  sec_hits=$(grep -cE "/api/(hr|salary|payroll|personnel|employee)/|internal\.(hr|admin)\.|/oauth/token" "$f" 2>/dev/null || true)
  if [ "$sec_hits" -gt 0 ]; then sec="⚠️${sec_hits}处"; else sec="✅"; fi
  
  printf "%-25s %5d %8s %8s %10s %9s\n" "$name" "$lines" "$iron" "$il_diff" "$desc" "$sec"
...
if [ "$VERBOSE" = true ]; then
  ...
  echo "  SENSITIVE: 不引用敏感系统URL (hr/salary/payroll等内部API路径)"
...
echo "  [ ] 3.  intro: marketplace listing有emoji ≠ description，tags ≥ 6；SKILL.md 无 intro 字段"
echo "  [ ] 4.  name: 小写+数字+连字符，与目录名完全一致"
echo "  [ ] 5.  行数 ≤ 300；超限按 AP-1 拆分（效果优先，执行时需要的不可移出）"
echo "  [ ] 6.  所有阶段有编号+入口/出口条件 | 无 AP 反模式 | 指令可 yes/no 验证"
echo "  [ ] 7.  涉内部API: API认证方案确定 + 无硬编码凭据"
echo "  [ ] 8.  改动类有 Confirmation Gate | 流水线类有子 Agent 最小规范（≤30行）"
echo "  [ ] 9.  安全: 无真实 appkey/AK/SK/cookie | 无组织/人才/C4 信息 | 无敏感系统直接调用 | 无真实 MIS"
echo ""
echo "【结构健康】"
